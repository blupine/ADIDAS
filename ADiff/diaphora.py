...
class CBinDiff:
    ...
    def search_small_differences(self, choose):
        ...
        for row in rows:
            ...
            if ratio >= 0.5:
                ea2 = row["ea2"]
                item = CChooser.Item(ea, name1, ea2, name2,
                                     "Nodes, edges, complexity and mnemonics with small differences", ratio, bb1, bb2)
                if ratio == 1.0:
                    self.best_chooser.add_item(item)
                else:
        ...
        if len(rows) > 0 and not self.all_functions_matched():
            for row in rows:
                ...
                if float(ratio) == 1.0 or (md1 != 0 and md1 == md2):
                    self.best_chooser.add_item(
                        CChooser.Item(ea, name, ea2, name, "Perfect match, same name", 1, bb1, bb2))
                else:
